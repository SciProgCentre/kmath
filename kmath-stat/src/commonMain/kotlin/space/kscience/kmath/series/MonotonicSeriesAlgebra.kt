...
public class MonotonicSeriesAlgebra<T, out A : Ring<T>, out BA : BufferAlgebra<T, A>, L : Comparable<L>>(
    ...
    public val Series<T>.labelRange: ClosedRange<L> get() = offsetToLabel(position)..offsetToLabel(position + size)
     ...
     */
    override fun Series<T>.getByLabelOrNull(label: L): T? = getByOffsetOrNull(floorOffset(label))
}